...
function MentorshipCard({ req, user, isAdmin, onApprove }) {
  const isMentor = req.mentor?._id === user?._id
  ...
  return (
    <div className="card animate-slide-up">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              {req.student?.dept} · Batch {req.student?.batch}
            </p>
          </div>
        </div>
      ...
      <div className="mt-3 flex items-center gap-2 text-sm text-slate-500">
        <span>Mentor:</span>
        <img
          src={req.mentor?.profilePic || `https://ui-avatars.com/api/?name=${encodeURIComponent(req.mentor?.name || 'M')}&background=dcfce7&color=16a34a&size=24`}
          alt=""
      ...
      {req.goals && (
        <div className="mt-2 bg-brand-50 rounded-xl p-3">
          ...
          <p className="text-sm text-brand-700">{req.goals}</p>
        </div>
      )}

      {/* ✅ UPDATED CONDITION */}
      {(isAdmin || isMentor) && req.status === 'pending' && (
        <div className="flex gap-2 mt-4 pt-4 border-t border-slate-100">
          <button
            onClick={() => onApprove(req._id, 'active')}
            className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-green-100 text-green-700 hover:bg-green-200 text-sm font-medium"
          >
            <CheckCircle2 size={14} /> Approve
          </button>

          <button
            onClick={() => onApprove(req._id, 'rejected')}
            className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-red-100 text-red-700 hover:bg-red-200 text-sm font-medium"
          >
      ...
      {req.status === 'active' && (
        <div className="mt-4 pt-4 border-t border-slate-100">
          <Link
            to={`/messages/${req.mentor?._id || req.student?._id}`}
